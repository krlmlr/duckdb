...
struct RLEScanState : public SegmentScanState {
	explicit RLEScanState(ColumnSegment &segment)
	    : handle(BufferManager::GetBufferManager(segment.GetDatabase()).Pin(segment.GetBlockHandle())), entry_pos(0),
	      position_in_entry(0), rle_count_offset(UnsafeNumericCast<uint32_t>(
	                                Load<uint64_t>(handle.GetDataMutable() + segment.GetBlockOffset()))),
	      data_pointer(reinterpret_cast<T *>(handle.GetDataMutable() + segment.GetBlockOffset() +
	                                         RLEConstants::RLE_HEADER_SIZE)),
	      index_pointer(
	          reinterpret_cast<rle_count_t *>(handle.GetDataMutable() + segment.GetBlockOffset() + rle_count_offset)),
	      max_entry_pos(
	          static_cast<idx_t>(reinterpret_cast<const_data_ptr_t>(handle.GetDataMutable() + segment.GetBlockSize()) -
...
void RLEScanPartialInternal(ColumnSegment &segment, ColumnScanState &state, idx_t scan_count, Vector &result,
	...
	while (result_offset < result_end) {
		const rle_count_t &run_end = scan_state.index_pointer[scan_state.entry_pos];
		...
		const T &element = scan_state.data_pointer[scan_state.entry_pos];
